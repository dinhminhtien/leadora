...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockGoogleSignInAuth.accessToken).thenReturn('google-access-token');
  ...
  group('loginWithGoogle & verifyOAuthSession', () {
    test('loginWithGoogle triggers Supabase signInWithOAuth', () async {
      ...
      verify(() => mockGoTrueClient.getOAuthSignInUrl(
        provider: supabase.OAuthProvider.google,
        redirectTo: any(named: 'redirectTo'),
      )).called(1);
    });

    test('verifyOAuthSession calls backend verify endpoint and returns AuthUser', () async {
      when(() => mockApiClient.get<AuthUser>(
        ApiPaths.oauthVerify,
        headers: any(named: 'headers'),
        decode: any(named: 'decode'),
      )).thenAnswer((invocation) async {
        final decode = invocation.namedArguments[#decode] as AuthUser Function(Object?);
        return decode({
          'id': '1',
      ...
      final result = await repository.verifyOAuthSession('supabase-access-token');

      expect(result.id, '1');
      expect(result.email, 'test@leadora.vn');
      
      verify(() => mockApiClient.get<AuthUser>(
        ApiPaths.oauthVerify,
  ...
  group('logout', () {
    test('clears everything and signs out Google/Supabase even if backend logout fails', () async {
