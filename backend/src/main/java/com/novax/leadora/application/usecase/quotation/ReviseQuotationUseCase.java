        ...
        public QuotationResponse execute(UUID parentId, ReviseQuotationRequest request) {
                ...
                if (!REVISABLE_STATUSES.contains(parent.getStatus())) {
                        throw new BusinessException("QUOTATION_NOT_REVISABLE",
                                        "Quotation cannot be revised from status " + parent.getStatus().name(),
                                        HttpStatus.CONFLICT);
                }
                ...
                BigDecimal discountPct = request.getDiscountPercent() != null ? request.getDiscountPercent()
                                : BigDecimal.ZERO;
                ...
                // The reviser owns the new version (drives SALES owner-scoping). Non-fatal if
                // unresolved.
