...
/**
 * UC-8.5 exception E6 — "The user may link the lead to the existing customer profile or cancel."
 *
 * <p><b>Why this needed to exist.</b> The duplicate check refused the conversion with a 409 and
 * stopped there, which implemented only the "or cancel" half of E6. A sales rep who found that
 * their lead was an existing guest had no way to record that fact: the lead stayed in the pipeline
 * forever, the guest's second enquiry was never attached to their history, and the only way out was
 * to invent a fake email so the duplicate check would pass — the exact split-record outcome
 * {@code CustomerDuplicatePolicy} was written to prevent.
 *
 * <p>The refusal already carried the existing customer's id in its {@code details} field, so the
 * missing piece was only ever this endpoint.
 *
 * <p><b>What it deliberately does not do:</b> it does not touch the customer. Linking says "this
 * enquiry came from that guest", not "replace that guest's record with this enquiry".
 */
@Service
...
public class LinkLeadToCustomerUseCase {
    ...
    public ConvertLeadResponse execute(UUID leadId, LinkLeadToCustomerRequest request) {
        ...
        // Same eligibility gate as a normal conversion: the lead ends up CONVERTED and locked
        ...
        // A customer already claimed by another lead must not be re-pointed: customers.lead_id is a
        // single column, so overwriting it would erase the first lead's link and leave that lead
        // CONVERTED but pointing at a customer that no longer points back. Rejecting is right —
