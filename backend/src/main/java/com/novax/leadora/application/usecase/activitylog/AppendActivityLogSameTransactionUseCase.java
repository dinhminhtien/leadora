...
public class AppendActivityLogSameTransactionUseCase {
    ...
    public void execute(ActivityLogCommand command) {
        ...
        try {
            ...
            log.info("Appended activity log entry (same tx): id={}, type={}, entity={}", entity.getId(),
                    entity.getActivityType(),
                    entity.getEntityId());
        } catch (Exception ex) {
            log.error("Failed to append activity log entry (same tx) for activityType={}, entityId={}. Error: {}",
                    command.getActivityType(), command.getEntityId(), ex.getMessage(), ex);
            throw ex; // Throw exception to ensure rollback
        }
    }
}
