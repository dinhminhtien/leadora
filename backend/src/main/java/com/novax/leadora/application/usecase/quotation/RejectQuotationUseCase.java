...
public class RejectQuotationUseCase {
    ...
    @Transactional
    public void execute(String token, RejectQuotationRequest request) {
        execute(token, request, null, null);
    }

    @Transactional
    public void execute(String token, RejectQuotationRequest request, String ipAddress, String userAgent) {
        ...
        QuotationAcceptanceLogEntity logEntity = QuotationAcceptanceLogEntity.builder()
                ...
                .customerNote(request.getReason())
                .ipAddress(ipAddress)
                .userAgent(userAgent)
                .build();
        ...
        eventPublisher.publishEvent(new QuotationRejectedEvent(this, savedQuotation, request.getReason()));

        log.info("Quotation {} rejected successfully by token.", savedQuotation.getQuotationId());
    }
}
