 ...
 *
 * <p>The customer is built from the <b>lead</b>, not from the request. See
 * {@link ConvertLeadRequest} for why the payload no longer carries the identity fields at all.
 *
 * <p>Everything after the validation block is delegated to {@link LeadConversionCompleter}, which
 * this use case shares with {@link LinkLeadToCustomerUseCase} (exception E6). Both end the same
 * way — the lead becomes an immutable CONVERTED snapshot, its SLA tracking stops, the action is
...
public class ConvertLeadUseCase {
    ...
    public ConvertLeadResponse execute(UUID leadId, ConvertLeadRequest request) {
        ...
        // 2. UC-8.5 step 2 / BR-02 — same owner-scoping as viewing: a Sales Staff may only convert
        ...
        // 3. UC-8.5 step 4 / E4 + BR-07 — is this lead eligible at all? Shared with the E6 link
        ...
        // 4. BR-09 — Individual or Corporate. The lead's own flag decides unless the caller
        ...
        // 5. The profile about to be created must be a usable one. Both rules read the LEAD's
        //    fields, because those are the fields the customer is built from — validating the
        //    request instead is what let a corporate customer be created with no company name.
        ...
        // 6. Two different leads can still describe the same person, so the duplicate check done
        //    when each LEAD was created protects nothing here. Same rule CreateCustomerUseCase
        //    applies, shared rather than copied — see CustomerDuplicatePolicy. On a hit this throws
        ...
        // 7. UC-8.5 step 6 — the customer profile, built from the lead. Owner and creator are
        //    inherited so the new profile lands in the same person's queue as the lead did.
        ...
        // 8. Keep leads.is_corporate consistent with the individual/organization decision that was
        //    just finalised, so the retained snapshot does not contradict the customer it produced.
        ...
        // 9. Steps 7-9 — mark converted, link both ways, stop SLA, write the audit entry.
