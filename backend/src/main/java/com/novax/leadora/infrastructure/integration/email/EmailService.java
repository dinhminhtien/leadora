...
public class EmailService {
    ...
    public void sendBookingConfirmationEmail(BookingEntity booking, List<BookingDetailEntity> details) {
        ...
        try {
            ...
            String htmlMsg = "<div style=\"font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; padding: 25px; border: 1px solid #e2e8f0; border-radius: 16px; background-color: #ffffff;\">"
                    + "  <div style=\"text-align: center; margin-bottom: 20px;\">"
                    + "    <h2 style=\"color: #1e293b; margin: 0; font-size: 22px; font-weight: 800; letter-spacing: -0.5px;\">Leadora</h2>"
                    + "    <p style=\"color: #64748b; font-size: 12px; margin: 4px 0 0 0;\">Smart Hotel Sales System</p>"
                    + "  </div>"
                    ...
                    + "      <tr><td style=\"font-weight: 600; width: 120px;\">Booking Code:</td><td style=\"color: #0f172a; font-weight: 700;\">" + escapeHtml(bookingCode) + "</td></tr>"
            ...
            helper.setText(htmlMsg, true);
            helper.setTo(toEmail);
            helper.setSubject("Booking Confirmed: " + bookingCode + " — Leadora Hotel");
            helper.setFrom(fromEmail);

            mailSender.send(mimeMessage);
            log.info("Booking confirmation HTML email successfully sent to {}", toEmail);
        } catch (Exception e) {
            log.error("Failed to send booking confirmation HTML email to {}", toEmail, e);
