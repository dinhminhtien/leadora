...
public interface SlaTrackingRepository extends JpaRepository<SlaTrackingEntity, UUID> {
  ...
  /**
   * The dashboard's SLA figures, narrowed to what one user is allowed to see
   * (BR-02).
   *
   * <p>
   * The dashboard used to read {@code findAll()} here, so a Sales Staff saw the
   * SLA
   * compliance of the whole company on a screen that carefully hid every
   * colleague's lead,
   * deal and task directly above it. An aggregate is still information.
   *
   * <p>
   * An SLA row names its subject polymorphically ({@code entity_type} +
   * {@code entity_id})
   * and carries no assignee of its own, so ownership has to be resolved by
   * joining whichever
   * subject it points at - the same four-way join {@code ChatAggregateRepository}
   * uses for
   * the chat snapshot, and it must stay in step with it. A null {@code :scope}
   * means "every
   * record", which is what a Manager or Admin gets.
   *
   * <p>
   * Returns {@code [status, startedAt, deadlineAt, resolvedAt]} - only the
   * columns the
  ...
  @Query(value = """
      ...
      SELECT st.status, st.started_at, st.deadline_at, st.resolved_at
        FROM sla_tracking st
        JOIN leads sl ON sl.lead_id = st.entity_id AND sl.assigned_user_id = CAST(:scope AS uuid)
       WHERE st.entity_type = 'LEAD' AND CAST(:scope AS uuid) IS NOT NULL
      UNION ALL
      SELECT st.status, st.started_at, st.deadline_at, st.resolved_at
        FROM sla_tracking st
        JOIN tasks stk ON stk.task_id = st.entity_id AND stk.assigned_user_id = CAST(:scope AS uuid)
       WHERE st.entity_type = 'TASK' AND CAST(:scope AS uuid) IS NOT NULL
      UNION ALL
      SELECT st.status, st.started_at, st.deadline_at, st.resolved_at
        FROM sla_tracking st
        JOIN quotations sq ON sq.quotation_id = st.entity_id AND sq.created_by = CAST(:scope AS uuid)
       WHERE st.entity_type = 'QUOTATION' AND CAST(:scope AS uuid) IS NOT NULL
      UNION ALL
      SELECT st.status, st.started_at, st.deadline_at, st.resolved_at
        FROM sla_tracking st
        JOIN bookings sb ON sb.booking_id = st.entity_id AND sb.assigned_user_id = CAST(:scope AS uuid)
       WHERE st.entity_type = 'BOOKING' AND CAST(:scope AS uuid) IS NOT NULL
      """, nativeQuery = true)
